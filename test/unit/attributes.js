...
test( "non-lowercase boolean attribute getters should not crash", function() {
	...
	// Two elements on purpose: jQuery.filter sends multi-element collections
	// through Sizzle's own ATTR matcher, which hands the raw (mixed-case) name
	// to the boolean attrHandle rather than the lowercased one
	var elems = jQuery( "<input checked required autofocus type='checkbox'>" +
		"<input checked required autofocus type='checkbox'>" );
	...
	jQuery.each({
		...
		try {
			strictEqual( elems.attr( original ), lowercased,
		...
		try {
			strictEqual( elems.filter( "[" + original + "]" ).length, 2,
				"The '" + original + "' attribute selector should match without recursing" );
		} catch ( e2 ) {
			ok( false, "The '" + original + "' attribute selector threw" );
		}
	});
});
